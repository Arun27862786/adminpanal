<?php if($display!=''){?><script>
 ...
 <input class="uk-table-shrink uk-hidden" name="id" type="text"  value="<?php echo $clients_details['id'];?>" >    
  ...
  <form method="post" class="uk-form-horizontal uk-margin-large" action="<?php $_SERVER['PHP_SELF'];?>" enctype="multipart/form-data"> 
   ...
   <div class="uk-margin">
        ...
        <div class="uk-form-controls">
            <input class="uk-input"   readonly="readonly" name="email" type="text"  value="<?php echo $clients_details['email'];?>">
        </div>
    </div>
    <div class="uk-margin">
        <label class="uk-form-label" for="form-horizontal-text">Name</label>
        <div class="uk-form-controls">
            <input class="uk-input" id="form-horizontal-text" name="name" type="text"  value="<?php echo $clients_details['name'];?>">
        </div>
    </div> 
    <div class="uk-margin">
        <label class="uk-form-label" for="form-horizontal-text">Address</label>
        <div class="uk-form-controls">
            <textarea class="uk-textarea" name="address" placeholder="Address"><?php echo html_entity_decode($clients_details['address']);?></textarea>
        </div>
    </div> 
...
<!-- 
    ...
    <?php if($clients_details['photo']==''){?>
    <span uk-icon="icon: cloud-upload"></span>
<?php }else{?>
<img src="<?php echo SITE_URL.'/uploads/logins/'.$clients_details['photo'];?>">
<?php }?></div>     
        <input class="uk-input" type="file" name="profile_img">   
...
<progress id="js-progressbar-profile" class="uk-progress" value="0" max="100" hidden></progress>
 ...
 <hr>
 <button class="uk-button uk-button-success uk-width-1-1 uk-border-rounded  uk-margin-small-bottom" name="Update_profile"><span uk-icon="icon:file-edit;"></span> Update Profile</button>   
 </form>   
 </div>
 <div>
