...
echo "<h3>Email Send Succssfully</h3>";
}
?>
    <div uk-grid> 
    ...
    <table class="uk-table uk-table-hover uk-table-middle uk-table-divider">
        ...
        if(is_array($invoice)){  
            foreach ($invoice as $invoices){
              $projects=$db->get_row('projects',array('id'=>$invoices['project_id'])); 


              $clients=$db->get_row('clients',array('id'=>$projects['client_id']));
             ?>    
           <tr>
                <td><?php echo $sno++;?></td>
    ...
    <div class="uk-inline">
        ...
        <div uk-dropdown="mode: click; boundary: ! .uk-button-group; boundary-align: true; pos: left-center"> 
            ...
            <li class="uk-nav-header"><a class="uk-text-success add_model" href="#modal_product"  id="<?php echo $invoices['id'];?>" uk-toggle  uk-icon="icon:strikethrough" >Payment</a></li>
             ...
             <li class="uk-nav-header"><a class="uk-text-success " href="<?php echo $link->link('invoice_pdf',backend,'?invoice_id='.$invoices['id']);?>" uk-icon="icon:happy" >View</a></li>

             <li class="uk-nav-header"><a  class="uk-text-primary"href="<?php echo $link->link('update_invoice',backend,'?update_id='.$invoices['id']);?>"  uk-icon="icon: file-edit">Edit</a></li>

             <li class="uk-nav-header"><a class="uk-text-danger"  href="<?php echo $link->link('invoices',backend,'?id='.$invoices['id']);?>" onclick="return confirm('Are you sure to Delete')"  uk-icon="icon: trash">Delete</a></li>
            </ul>
        </div>
    </div>
</div></li>
  ...
  <form class="model_data" action="<?php $_SERVER['PHP_SELF'];?>" method="post">
     ...
     <div class="uk-margin"> 
      <label class="uk-form-label" for="form-stacked-text">Date</label>
       <input class="uk-input" type="datetime-local" name="paid_date" >
        </div>
        <div class="uk-margin"> 
