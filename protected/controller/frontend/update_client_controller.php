...
if (isset($_REQUEST['id']) && $_REQUEST['id']!=""){
  ...
  $get_data = $db->get_row('clients',array('id'=>$_REQUEST['id']));
...
if(isset($_POST['update_client']))
{
 ...
 $update=$db->update('clients',array('name'=>$name,'address'=>$address),array('id'=>$get_data['id']));
             	}
	...
	if($update)
	{
	    ...
	    $color="success"; return;
     	 ...
     	 $display="<span uk-icon=\'icon: warning\'></span>Somethingg wrong.";
        $color="danger"; return;
     }
  }else{
  	   if(empty($name)){
    ...
    if(isset($_FILES["profile_img"]) && $_FILES["profile_img"]["error"] == 0){
       ...
       $maxsize = 5 * 1024 * 1024;
        ...
        if(in_array($filetype, $allowed)){ 
            ...
            $filen =$get_data['photo'];
          

                move_uploaded_file($_FILES["profile_img"]["tmp_name"], "uploads/logins/" . $filename);
               // echo "Your file was uploaded successfully.";
             $update=$db->update('clients',array('name'=>$name,'address'=>$address,'photo'=>$filename),array('id'=>$get_data['id']));
             	}
...
}elseif(isset($_POST['submit_changepassword']))
{
    ...
    $verify_pass=password_verify($oldpassword,$get_data['password']);
   ...
   elseif($fv->emptyfields(array('password'=>$oldpassword),NULL))
	...
	else
	{
		...
		$update=$db->update('clients',array('password'=>$encrypt_password),array('id'=>$get_data['id']));
	   	}
	if($update)
	{
