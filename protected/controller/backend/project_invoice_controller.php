...
if (isset($_REQUEST['project_id']) && $_REQUEST['project_id']!=""){
    $pro=$db->get_row('projects',array('id'=>$_REQUEST['project_id']));
    $Client_name=$db->get_row('clients',array('id'=>$pro['client_id']));
 }
...
if( $insert_row){
      $display="<span uk-icon=\'icon: warning\'></span>  Sccessfull Add Invoice.";  
      $color="success"; 
}else{
   $display="<span uk-icon=\'icon: warning\'></span>  Problem for Insertion.";  
    $color="success"; 
}
